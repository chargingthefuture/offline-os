/* Yard Jockey Call Sheet — service worker. Precaches the shell (incl. shared assets) so it
 * runs fully offline. Bump VERSION to push updates. */
var VERSION = 'yard-call-v1';
var SHELL = [
  './',
  './index.html',
...
self.addEventListener('activate', function (e) {
  e.waitUntil(caches.keys().then(function (names) {
    return Promise.all(names.map(function (n) {
      if (n !== VERSION && n.indexOf('yard-call') === 0) return caches.delete(n);
    }));
